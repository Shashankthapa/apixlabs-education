...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    return (
      <>
        <div
          className={`${className} flex items-center justify-center rounded-tl-[10px] rounded-bl-[10px] text-base ${shapes[shape] || ""} ${variants[variant]?.[color as keyof (typeof variants)[typeof variant]] || variants[variant] || ""} ${sizes[size] || ""}`}
        >
          {!!label && label}
          {!!prefix && prefix}
